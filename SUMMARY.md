# Summary

* [Android Studio 使用艺术 介绍](README.md)
* [Android Studio 发布版本](android-studio-发布版本.md)
    * [Android Studio 2.2](android-studio-22.md)
* [前言](前言.md)
* [第一章 Android Studio 入门](第一章 Android Studio 入门.md)
    * [下载](下载.md)
    * [Windows 环境配置](windows-环境配置.md)
    * [Mac OS 环境配置](mac-os-环境配置.md)
    * [Linux 环境配置](linux-环境配置.md)
    * [版本更新](版本更新.md)
    * [Android Studio 项目和模块](android-studio-项目结构.md)
* [第二章 新建](新建.md)
    * [创建项目](创建项目.md)
    * [创建模块](创建模块.md)
    * [创建库](创建库.md)
    * [将Android Library打包成aar](将android-library打包成aar.md)
    * [导入Android Studio项目](导入android-studio项目.md)
    * [导入eclipse项目](导入eclipse项目.md)
    * [导入Jar\/AAR](导入jaraar.md)
    * [新建类和文件](新建类和文件.md)
    * [增加so文件](增加so文件.md)
    * [创建Fragment文件](创建fragment文件.md)
    * [创建service文件](创建service文件.md)
    * [创建自定义组件](创建自定义组件.md)
    * [创建app widget](创建app-widget.md)
    * [创建可编译的资源文件](创建可以资源文件.md)
    * [创建AIDL](创建aidl.md)
    * [创建Android文件夹](创建android文件夹.md)
    * 将Android Library打包成aar
* [第三章 布局](第三章-布局.md)
    * [认识布局](认识布局.md)
    * [组件列表](组件列表.md)
    * [属性](属性.md)
    * [Widget组件](widget组件.md)
    * [ConstraintLayout约束](constraintlayout约束.md)
* [第四章 管理](第四章-管理.md)
    * [项目窗口](项目窗口.md)
    * [类结构](类结构.md)
    * [捕获界面](捕获界面.md)
    * [文件管理](文件管理.md)
    * [切换文件编码](切换文件编码.md)
    * [文件同步](文件同步.md)
    * [收藏夹](收藏夹.md)
    * [TODO](todo.md)
    * [Build Variants（构建变种版本）](build-variants（构建变种版本）.md)
* [第五章 编辑](第五章-编辑.md)
    * [复制\/粘贴\/剪切\/撤销\/重做](撤销重做剪切粘贴.md)
    * [查找替换](查找替换.md)
    * [大小写替换](大小写替换.md)
    * [Macros（宏）](macros（宏）.md)
    * [使用列选择模式](使用列选择模式.md)
    * [扩大\/缩小选择范围](扩大缩小选择范围.md)
    * [合并两行内容](合并两行内容.md)
    * [自动补全当前的语句](自动补全当前的语句.md)
    * [缩排](缩排.md)
    * [切换大小写字母](切换大小写字母.md)
* [第六章 窗口视图](第六章-窗口视图.md)
    * [工具窗口管理](工具窗口管理.md)
    * [操作窗口管理](操作窗口管理.md)
    * [查看定义](查看定义.md)
    * [查看同胞元素](查看同胞元素.md)
    * [查看方法的参数](查看方法的参数.md)
    * [查看表达式的类型](查看表达式的类型.md)
    * [查看源码](查看源码.md)
    * [查看最近改变过的文件](查看最近改变过的文件.md)
    * [查看最近的改动](查看最近的改动.md)
    * [对比任意文件](对比任意文件.md)
    * [快速切换视图模式](快速切换视图模式.md)
    * [快速切换全屏](快速切换全屏.md)
    * [设置当前编辑器是否显示行号](设置当前编辑器是否显示行号.md)
    * [使用演示模式](使用演示模式.md)
    * [使用全屏模式](使用全屏模式.md)
    * [使用免打扰模式](使用免打扰模式.md)
    * [工具窗口间快速切换](工具窗口间快速切换.md)
* [第七章 导航](第七章-导航.md)
    * [搜索并打开某个类文件](搜索并打开某个类文件.md)
    * [搜索并打开某个文件](搜索并打开某个文件.md)
    * [搜索并打开某个文件或方法](搜索并打开某个文件或方法.md)
    * [快速跳转到某一行](快速跳转到某一行.md)
    * [使用自定义代码块](使用自定义代码块.md)
    * [快速跳转到光标的历史位置](快速跳转到光标的历史位置.md)
    * [快速跳转到编辑过的位置](快速跳转到编辑过的位置.md)
    * [标记书签](标记书签.md)
    * [管理书签](管理书签.md)
    * [快速跳转到导航栏](快速跳转到导航栏.md)
    * [快速跳转到声明](快速跳转到声明.md)
    * [快速跳转到类型声明](快速跳转到类型声明.md)
    * [快速跳转到实现](快速跳转到实现.md)
    * [快速跳转到父类](快速跳转到父类.md)
    * [在快速类和被测试类之间快速跳转](在快速类和被测试类之间快速跳转.md)
    * [查看相关联的文件](查看相关联的文件.md)
    * [查看文件结构](查看文件结构.md)
    * [查看类的层次结构图](查看类的层次结构图.md)
    * [查看方法类型的层次结构图](查看方法类型的层次结构图.md)
    * [查看方法调用层次结构](查看方法调用层次结构.md)
    * [快速跳转到错误代码位置](快速跳转到错误代码位置.md)
    * [方法间前后跳转](方法间前后跳转.md)
    * [使用翻页](使用翻页.md)
    * [选择当前文件在哪里显示](选择当前文件在哪里显示.md)
    * [光标快速跳转到编辑器](光标快速跳转到编辑器.md)
* [第八章 编码](第八章-编码.md)
    * [覆写或者实现方法](覆写或者实现方法.md)
    * [实现接口方法](实现接口方法.md)
    * [实现代理方法](实现代理方法.md)
    * [生成构造函数](生成构造函数.md)
    * [生成Getter和Setter方法](生成getter和setter方法.md)
    * [覆写equals和hashcode方法](覆写equals和hashcode方法.md)
    * [快速覆写toString方法](快速覆写tostring方法.md)
    * [插入版权信息](插入版权信息.md)
    * [提取或删除语句](提取或删除语句.md)
    * [自动补全提示](自动补全提示.md)
    * [代码补全](代码补全.md)
    * [循环扩展词](循环扩展词.md)
    * [展开或折叠代码](展开或折叠代码.md)
    * [代码模板](代码模板.md)
    * [注释](注释.md)
    * [格式化代码](格式化代码.md)
* [第九章 检查](第九章-检查.md)
    * [了解Lint](了解lint.md)
    * [Lint命令使用](lint命令使用.md)
* [第十章 重构](第十章-重构.md)
    * [重命名](重命名.md)
    * [提炼方法](提炼方法.md)
    * [提炼方法对象](提炼方法对象.md)
    * [更改方法签名](更改方法签名.md)
    * [迁移变量类型](迁移变量类型.md)
    * [转成静态方法](转成静态方法.md)
    * [静态方法转为实例方法](静态方法转为实例方法.md)
    * [移动类](移动类.md)
    * [移动静态方法](移动静态方法.md)
    * [移动静态字段](移动静态字段.md)
    * [安全删除](安全删除.md)
    * [提炼为变量](提炼为变量.md)
    * [提炼为常量](提炼为常量.md)
    * [提炼字段](提炼字段.md)
    * [提炼参数](提炼参数.md)
    * [提炼委托](提炼委托.md)
    * [提炼接口](提炼接口.md)
    * [提炼父类](提炼父类.md)
    * [内联方法](内联方法.md)
    * [内联临时变量](内联临时变量.md)
    * [查找并替换重复代码](查找并替换重复代码.md)
    * [反转布尔值](反转布尔值.md)
    * [把成员拉到父类](把成员拉到父类.md)
    * [把成员推到子类](把成员推到子类.md)
    * [尽可能使用接口](尽可能使用接口.md)
    * [使用委托替换继承](使用委托替换继承.md)
    * [移除中间人](移除中间人.md)
    * [包装方法返回值](包装方法返回值.md)
    * [将匿名类转成内部类](将匿名类转成内部类.md)
    * [封装字段](封装字段.md)
    * [使用查询替换临时变量](使用查询替换临时变量.md)
    * [使用工厂方法替换构造方法](使用工厂方法替换构造方法.md)
    * [使用构建器替换构造方法](使用构建器替换构造/b方法.md)
    * [泛型化](泛型化.md)
    * [迁移](迁移.md)
* [第十一章 打包构建（Gradle）](第十一章-打包构建.md)
    * [认识Gradle](认识gradle.md)
    * [Gradle中依赖的仓库](gradle中依赖的仓库.md)
    * [配置Gradle环境](配置gradle环境.md)
    * [Gradle Wrapper](gradle-wrapper.md)
    * [初始化构建环境](初始化构建环境.md)
    * [解决Gradle下载太慢的问题](解决gradle下载太慢的问题.md)
    * [查看和执行Gradle任务](查看和执行gradle任务.md)
    * [常用Gradle任务](常用gradle任务.md)
    * [Gradle工具窗口](gradle工具窗口.md)
    * [构建项目和模块](构建项目和模块.md)
    * [设置自动编译项目](设置自动编译项目.md)
    * [重新构建项目](重新构建项目.md)
    * [Make Project跟Rebuild Project的区别](make-project跟rebuild-project的区别.md)
    * [清理项目](清理项目.md)
    * [Gradle Script](gradle-script.md)
    * [Gradlew配置文件:gradle-wrapper.properties](gradlew配置文件gradle-wrapperproperties.md)
    * [项目全局配置文件:settings.gradle](项目全局配置文件settingsgradle.md)
    * [本地属性配置文件:local.properties](本地属性配置文件localproperties.md)
    * [Gradle配置文件:gradle.properties](gradle配置文件gradleproperties.md)
    * [代码混淆规则配置文件:proguard-rules.pro](代码混淆规则配置文件proguard-rulespro.md)
    * [项目构建配置文件build.gradle](项目构建配置文件buildgradle.md)
    * [模块构建配置文件:build.gradle](模块构建配置文件buildgradle.md)
    * [项目结构中配置模块构建](项目结构中配置模块构建.md)
    * [配置应用程序属性](配置应用程序属性.md)
    * [配置应用程序签名](配置应用程序签名.md)
    * [配置应用程序的特性](配置应用程序的特性.md)
    * [配置应用程序的构建类型](配置应用程序的构建类型.md)
    * [配置应用程序的依赖](配置应用程序的依赖.md)
    * [打签名包](打签名包.md)
    * [多渠道打包](多渠道打包.md)
    * [配置开发者服务](配置开发者服务.md)
* [第十二章 运行调试](第十二章-运行调试.md)
    * [运行和调试配置](运行和调试配置.md)
    * [Android应用程序配置](android应用程序配置.md)
    * [断点](断点.md)
    * [调试工具](调试工具.md)
    * [计算表达式](计算表达式.md)
    * [关联调试到Android进程](关联调试到android进程.md)
    * [配置和运行单元测试](配置和运行单元测试.md)
    * [使用命令行运行单元测试](使用命令行运行单元测试.md)
    * [配置Android单元测试](配置android单元测试.md)
* [第十三章 工具](第十三章-工具.md)
    * [任务](任务.md)
    * [变更列表](变更列表.md)
    * [管理上下文](管理上下文.md)
    * [生成JavaDoc](生成javadoc.md)
    * [将当前文件保存为模板](将当前文件保存为模板.md)
    * [创建命令行启动](创建命令行启动.md)
    * [IDE Scripting Console](ide-scripting-console.md)
    * [管理Android SDK](管理android-sdk.md)
    * [管理Android虚拟机](管理android虚拟机.md)
    * [Android模拟器](android模拟器.md)
    * [即时运行](即时运行.md)
    * [Android监视器](android监视器.md)
    * [截图](截图.md)
    * [录像](录像.md)
    * [捕获系统信息](捕获系统信息.md)
    * [布局解析](布局解析.md)
    * [Logcat监视器](logcat监视器.md)
    * [内存监视器](内存监视器.md)
    * [CPU监视器](cpu监视器.md)
    * [网络监视器](网络监视器.md)
    * [GPU监视器](gpu监视器.md)
* [第十四章 版本控制](第十四章-版本控制.md)
    * [Git偏好设置](Git偏好设置.md)
    * [配置Github](配置github.md)
    * [从Github克隆代码](从github克隆代码.md)
    * [将本地项目共享到GitHub](将本地项目共享到github.md)
    * [查看本地变更历史](查看本地变更历史.md)
    * [Git添加文件](git添加文件.md)
    * [Git提交变更](git提交变更.md)
    * [Git文件逐行追溯](git文件逐行追溯.md)
    * [显示当前修订版本](显示当前修订版本.md)
    * [Git文件比较](git文件比较.md)
    * [Git撤销操作](git撤销操作.md)
    * [Git版本回退](git版本回退.md)
    * [Git查看提交历史](git查看提交历史.md)
    * [Git分支管理](git分支管理.md)
    * [Git创建标签](git创建标签.md)
* [第十五章 偏好设置](第十五章-偏好设置.md)
    * [第一部分 外观与行为](第一部分-外观与行为.md)
        * [外观](外观.md)
            * [设置Android Studio的主题](设置android-studio的主题.md)
            * [设置Android Studio的字体和大小](设置android-studio的字体和大小.md)
            * [设置工具提示的延迟时间](设置工具提示的延迟时间.md)
            * [在状态栏显示内存状态](在状态栏显示内存状态.md)
            * [调整演示模式的字体大小](调整演示模式的字体大小.md)
        * [菜单和工具栏](菜单和工具栏.md)
            * [对菜单选项和工具栏的工具进行增删攺](对菜单选项和工具栏的工具进行增删攺.md)
        * 系统设置
            * [设置启动Android Studio时是否自动打开项目](设置启动android-studio时是否自动打开项目.md)
            * [设置退出Android Studio时是否弹出确认提示](设置退出android-studio时是否弹出确认提示.md)
            * [设置打开一个项目时的打开方式](设置打开一个项目时的打开方式.md)
            * [设置文件自动保存的时间](设置文件自动保存的时间.md)
            * [设置密码存储策略](设置密码存储策略.md)
            * [设置HTTP代理](设置http代理.md)
            * [设置Android Studio版本更新规则](设置android-studio版本更新规则.md)
            * [设置是否让Google统计你的使用信息](设置是否让google统计你的使用信息.md)
            * [管理Android SDK平台](管理android-sdk平台.md)
            * [使用单独的SDK管理窗口来管理SDK](使用单独的sdk管理窗口来管理sdk.md)
            * [管理Android SDK开发工具](管理android-sdk开发工具.md)
            * [管理Android SDK更新站点](管理android-sdk更新站点.md)
        * [文件颜色](文件颜色.md)
        * [范围](范围.md)
        * [通知](通知.md)
        * [快速列表](快速列表.md)
        * [路径变量](路径变量.md)
    * 第二部分 键盘映射
        * [Android Studio中使用Eclipse快捷键](android-studio中使用eclipse快捷键.md)
        * [自定义keymap](自定义keymap.md)
    * [第三部分 编辑器](第三部分-编辑器.md)
        * [常规设置](常规设置.md)
            * [编辑器常规设置](编辑器常规设置.md)
                * [设置点击编辑器光标定位在一行的结尾或定位在点击的位置](设置点击编辑器光标定位在一行的结尾或定位在点击的位置.md)
                * [设置鼠标悬停在元素上会显示文档提示](设置鼠标悬停在元素上会显示文档提示.md)
                * [设置是否自动换行](设置是否自动换行.md)
                * [设置粘贴历史记录的个数](设置粘贴历史记录的个数.md)
                * [置记录最近打开项目的个数](置记录最近打开项目的个数.md)
            * [设置自动导入](设置自动导入.md)
                * [设置粘帖时自动导入包](设置粘帖时自动导入包.md)
                * [设置自动导入需要的包](设置自动导入需要的包.md)
                * [设置是否弹出导入提示](设置是否弹出导入提示.md)
            * [设置外观](设置外观.md)
                * [设置编辑器一直显示行号](设置编辑器一直显示行号.md)
                * [设置编辑器显示方法分隔符](设置编辑器显示方法分隔符.md)
                * [设置编辑器显示空格](设置编辑器显示空格.md)
                * [设置编辑器显示缩进向导](设置编辑器显示缩进向导.md)
            * [设置代码补全](设置代码补全.md)
                * [设置自动补全时是否区分大小写](设置自动补全时是否区分大小写.md)
                * [加快自动弹出代码补全提示的速度](加快自动弹出代码补全提示的速度.md)
                * [关闭自动弹出代码补全提示](关闭自动弹出代码补全提示.md)
                * [设置查看方法参数信息的时候显示方法签名](设置查看方法参数信息的时候显示方法签名.md)
            * [设置代码折叠](设置代码折叠.md)
            * [设置控制台](设置控制台.md)
            * [设置编辑器标签](设置编辑器标签.md)
                * [设置用星号标记修攺过的文件标签](设置用星号标记修攺过的文件标签.md)
                * [设置打开的文件标签可以多行显示](设置打开的文件标签可以多行显示.md)
                * [设置文件标签的显示位置](设置文件标签的显示位置.md)
                * [设置打开的文件标签超过一定数量时的关闭规则](设置打开的文件标签超过一定数量时的关闭规则.md)
            * [设置后缀补全](设置后缀补全.md)
            * [设置智能键](设置智能键.md)
                * [开启使用驼峰单词](开启使用驼峰单词.md)
        * [颜色和字体](颜色和字体.md)
            * [切换编辑器配色方案](切换编辑器配色方案.md)
            * [为编辑器添加自定义配色方案](为编辑器添加自定义配色方案.md)
            * [设置代码的字体和大小](设置代码的字体和大小.md)
            * [设置是否显示条标和条标的显示颜色](设置是否显示条标和条标的显示颜色.md)
            * [设置控制台的颜色](设置控制台的颜色.md)
    * [第四部分 插件](第四部分-插件.md)
    * [第五部分 版本控制](第五部分-版本控制.md)
    * [第六部分 构建,执行,部署](第六部分-构建执行部署.md)
        * [构建工具](构建工具.md)
            * [设置开启Gradle离线工作模式](设置开启gradle离线工作模式.md)
        * [编译](编译.md)
            * 设置自动编译项目
            * [设置并行编译](设置并行编译.md)
            * [调整编译内存大小](调整编译内存大小.md)
    * [第七部分 语言与框架](第七部分-语言与框架.md)
    * [第八部分 工具](第八部分-工具.md)
* [第十六章 插件](第十六章-插件.md)
    * [插件下载安装](插件下载安装.md)
    * [常用插件](常用插件.md)
    * [插件开发](插件开发.md)
* [第十七章 帮助](第十七章-帮助.md)
    * [搜索菜单选项](搜索菜单选项.md)
    * [查找操作功能](查找操作功能.md)
    * [效率指南](效率指南.md)
* [附录 快捷键](附录-快捷键.md)

